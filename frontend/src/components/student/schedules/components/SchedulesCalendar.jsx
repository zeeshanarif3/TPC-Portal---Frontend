...
export default function SchedulesCalendar({
  ...
  /*
   * Find the first valid schedule date.
   */
   ...
   * Initial week:
   *
   * 1. selectedDate if provided
   * 2. otherwise first schedule date
   * 3. otherwise today
   */
  const initialCalendarDate =
    selectedDate
      ? new Date(selectedDate)
      : firstScheduleDate || new Date();

  const [selectedWeekStart, setSelectedWeekStart] =
    useState(() =>
      getMonday(initialCalendarDate)
    );

  /*
   * Current month shown by mini calendar.
   */
  ...
  /*
   * When schedules arrive asynchronously,
   * move calendar to the first schedule.
   */
  useEffect(() => {
    if (selectedDate) {
  ...
  }, [selectedDate, firstScheduleDate]);
  ...
  const getTrainerName = (schedule) => {
     ...
     * }
     */
    if (schedule?.trainer?.name) {
      return schedule.trainer.name;
    }

    const trainerId = normalizeId(
      schedule?.trainer ||
        schedule?.trainerId
    );
  ...
  const generateTimeSlots = () => {
    ...
    schedules.forEach((schedule) => {
      const start = parseTimeToHour(
        schedule.startTime
      );

      const end = parseTimeToHour(
        schedule.endTime
      );
      ...
      <div className="mini-calendar">
        ...
        <div className="calendar-dates-wrapper">
          {(() => {
            ...
            const selectedWeekStartFormatted =
              ymd(selectedWeekStart);
            ...
            return weeks.map(
              (week, wi) => {
                ...
                return (
                  ...
                  >
                    {isSelectedWeek && (
                      <span className="active-week-dot" />
                    )}
...
//   /*
//    * Gets trainer name from the NEW structure:
//    *
//    * trainer: {
//    *   _id: "...",
//    *   name: "trainer2"
//    * }
//    *
//    * Also keeps compatibility with trainerMap / trainerId.
//    */
//   const getTrainerName = (schedule) => {
//     // New API structure
//     if (schedule?.trainer?.name) {
...
//     // If trainer itself is a string/object ID
//     const trainerId = normalizeId(
//       schedule?.trainer || schedule?.trainerId
//     );
...
//   /*
//    * Gets schedules for a particular:
//    *
//    * Day + Hour
//    */
...
//           trainer: getTrainerName(schedule),
...
//           attendanceTaken: schedule.attendanceTaken,
...
//                   }
//                 >
//                   {getSchedulesForSlot(day, hour).map(
//                     (schedule) => (
//                       <div
//                         key={schedule.id}
//                         className={`Schedulee-card ${
...
//             /*
//              * Dates that have at least one schedule.
//              */
...
// //                       <div className="card-trainer">Trainer: {schedule.trainer}</div>
// //                       <div className="card-time">
...
// //             const selectedDateFormatted = selectedDate ? ymd(new Date(selectedDate)) : null;
