import { useStu } from "../../../hooks/useStu";
import "./PerformancePage.css";

import {
    RadialBarChart,
    RadialBar,
    ResponsiveContainer,
    BarChart,
    Bar,
    XAxis,
    YAxis,
    CartesianGrid,
    Tooltip,
} from "recharts";


function getAttendanceTier(pct) {
    if (pct >= 90) return { label: "Excellent", tone: "excellent" };
    if (pct >= 75) return { label: "Good", tone: "good" };
    if (pct >= 60) return { label: "Needs attention", tone: "fair" };
    return { label: "At risk", tone: "risk" };
}


function getScoreTier(pct) {
    if (pct >= 90) return "gold";
    if (pct >= 75) return "good";
    if (pct >= 60) return "fair";
    return "risk";
}


function getOverallGrade(avgPct) {
    if (avgPct >= 90) return "A+";
    if (avgPct >= 80) return "A";
    if (avgPct >= 70) return "B";
    if (avgPct >= 60) return "C";
    return "D";
}


function initials(name = "") {
    return name
        .split(" ")
        .map((part) => part[0])
        .filter(Boolean)
        .slice(0, 2)
        .join("")
        .toUpperCase();
}


export default function PerformancePage({ token }) {

    const {
        myPerformance
    } = useStu(token);

    const performance = myPerformance?.data;


    if (!performance) {
        return (
            <div className="performance-page">

                <div className="performance-loading">
                    <div className="loading-ring" />
                    <span>
                        Loading your performance...
                    </span>
                </div>

            </div>
        );
    }


    const {
        attendance,
        assessments,
        feedback
    } = performance;


    /*
     * ------------------------------------------------
     * BASIC CALCULATIONS
     * ------------------------------------------------
     */

    const attendanceTier =
        getAttendanceTier(attendance.percentage);

    const overallGrade =
        getOverallGrade(
            assessments.averageScorePercent
        );


    /*
     * ------------------------------------------------
     * ATTENDANCE DATA
     * ------------------------------------------------
     */

    const attendanceChartData = [
        {
            name: "Attendance",
            value: attendance.percentage,
        },
    ];


    const attendanceBreakdownData = [
        {
            name: "Classes",
            Present: attendance.present,
            Absent: attendance.absent,
        },
    ];


    /*
     * ------------------------------------------------
     * ASSESSMENT DATA
     * ------------------------------------------------
     */

    const submissions =
        assessments.submissions || [];


    const assessmentChartData =
        submissions.map((submission, index) => ({
            name: `A${index + 1}`,
            title: submission.title,
            score:
                submission.percentageScore ??
                submission.percentage ??
                0,
        }));


    const assessmentRangeData = [
        {
            range: "90–100",
            count: submissions.filter(
                (s) =>
                    (s.percentageScore ?? 0) >= 90
            ).length,
        },
        {
            range: "75–89",
            count: submissions.filter(
                (s) => {
                    const score =
                        s.percentageScore ?? 0;

                    return (
                        score >= 75 &&
                        score < 90
                    );
                }
            ).length,
        },
        {
            range: "60–74",
            count: submissions.filter(
                (s) => {
                    const score =
                        s.percentageScore ?? 0;

                    return (
                        score >= 60 &&
                        score < 75
                    );
                }
            ).length,
        },
        {
            range: "Below 60",
            count: submissions.filter(
                (s) =>
                    (s.percentageScore ?? 0) < 60
            ).length,
        },
    ];


    /*
     * ------------------------------------------------
     * EXTRA SCORE INFORMATION
     * ------------------------------------------------
     */

    const lowestScore =
        submissions.length > 0
            ? Math.min(
                  ...submissions.map(
                      (submission) =>
                          submission.percentageScore ??
                          submission.percentage ??
                          0
                  )
              )
            : 0;


    const highestScore =
        submissions.length > 0
            ? Math.max(
                  ...submissions.map(
                      (submission) =>
                          submission.percentageScore ??
                          submission.percentage ??
                          0
                  )
              )
            : 0;


    /*
     * ------------------------------------------------
     * FEEDBACK DATA
     * ------------------------------------------------
     */

    const feedbackRatingData =
        [1, 2, 3, 4, 5].map((rating) => ({
            rating: `${rating}★`,
            count: feedback.filter(
                (item) =>
                    Number(item.rating) === rating
            ).length,
        }));


    const feedbackAverage =
        feedback.length > 0
            ? (
                  feedback.reduce(
                      (sum, item) =>
                          sum +
                          Number(item.rating || 0),
                      0
                  ) / feedback.length
              ).toFixed(1)
            : "—";


    return (

        <div className="performance-page">


            {/* =====================================================
                HERO
            ====================================================== */}

            <header className="performance-hero">

                <div className="hero-copy">

                    <span className="hero-eyebrow">
                        Performance overview
                    </span>

                    <h1>
                        My Performance
                    </h1>

                    <p>
                        Track your attendance,
                        assessments and trainer
                        feedback in one place.
                    </p>

                </div>


                <div
                    className="hero-grade"
                    title="Overall grade based on your average assessment score"
                >

                    <span className="hero-grade-label">
                        Overall grade
                    </span>

                    <span className="hero-grade-value">
                        {overallGrade}
                    </span>

                </div>

            </header>



            {/* =====================================================
                KPI CARDS
            ====================================================== */}

            <section className="kpi-grid">


                {/* Total classes */}

                <div className="kpi-card">

                    <div className="kpi-icon accent-blue">
                        🗓️
                    </div>

                    <span className="kpi-label">
                        Total Classes
                    </span>

                    <span className="kpi-value">
                        {attendance.totalClasses}
                    </span>

                    <span className="kpi-subtitle">
                        Overall classes
                    </span>

                </div>



                {/* Attendance */}

                <div
                    className={`kpi-card accent-${attendanceTier.tone}`}
                >

                    <div
                        className={`kpi-icon accent-${attendanceTier.tone}`}
                    >
                        ✓
                    </div>

                    <span className="kpi-label">
                        Attendance
                    </span>

                    <span className="kpi-value">
                        {attendance.percentage}%
                    </span>

                    <span
                        className={`kpi-subtitle tone-${attendanceTier.tone}`}
                    >
                        {attendanceTier.label}
                    </span>

                </div>



                {/* Assessments */}

                <div className="kpi-card accent-gold">

                    <div className="kpi-icon accent-gold">
                        📝
                    </div>

                    <span className="kpi-label">
                        Assessments
                    </span>

                    <span className="kpi-value">
                        {assessments.totalAttempted}
                    </span>

                    <span className="kpi-subtitle">
                        Attempted
                    </span>

                </div>



                {/* Average */}

                <div className="kpi-card accent-gold">

                    <div className="kpi-icon accent-gold">
                        📊
                    </div>

                    <span className="kpi-label">
                        Average Score
                    </span>

                    <span className="kpi-value">
                        {assessments.averageScorePercent}%
                    </span>

                    <span className="kpi-subtitle">
                        Overall average
                    </span>

                </div>



                {/* Best */}

                <div className="kpi-card accent-gold">

                    <div className="kpi-icon accent-gold">
                        🏆
                    </div>

                    <span className="kpi-label">
                        Best Score
                    </span>

                    <span className="kpi-value">
                        {assessments.bestScorePercent}%
                    </span>

                    <span className="kpi-subtitle">
                        Personal best
                    </span>

                </div>



                {/* Lowest */}

                <div className="kpi-card">

                    <div className="kpi-icon accent-blue">
                        📉
                    </div>

                    <span className="kpi-label">
                        Lowest Score
                    </span>

                    <span className="kpi-value">

                        {submissions.length
                            ? `${lowestScore}%`
                            : "—"}

                    </span>

                    <span className="kpi-subtitle">
                        Lowest assessment
                    </span>

                </div>

            </section>



            {/* =====================================================
                ATTENDANCE DASHBOARD
            ====================================================== */}

            <section className="dashboard-grid">


                {/* Attendance radial chart */}

                <div className="dashboard-card">

                    <div className="section-header">

                        <h2>
                            Attendance Overview
                        </h2>

                        <p>
                            Your overall class attendance
                        </p>

                    </div>


                    <div className="attendance-panel">


                        <div className="attendance-chart-wrap">

                            <ResponsiveContainer
                                width="100%"
                                height={250}
                            >

                                <RadialBarChart
                                    cx="50%"
                                    cy="50%"
                                    innerRadius="68%"
                                    outerRadius="90%"
                                    barSize={16}
                                    startAngle={90}
                                    endAngle={-270}
                                    data={attendanceChartData}
                                >

                                    <RadialBar
                                        fill="var(--accent)"
                                        background
                                        dataKey="value"
                                        cornerRadius={12}
                                    />


                                    <text
                                        x="50%"
                                        y="47%"
                                        textAnchor="middle"
                                        dominantBaseline="middle"
                                        className="attendance-chart-value"
                                    >
                                        {attendance.percentage}%
                                    </text>


                                    <text
                                        x="50%"
                                        y="61%"
                                        textAnchor="middle"
                                        dominantBaseline="middle"
                                        className="attendance-chart-label"
                                    >
                                        Attendance
                                    </text>

                                </RadialBarChart>

                            </ResponsiveContainer>

                        </div>



                        <div className="performance-attendance-stats">


                            <div className="attendance-stat">

                                <span className="dot dot-good" />

                                <div>

                                    <span className="attendance-stat-value">
                                        {attendance.present}
                                    </span>

                                    <span className="attendance-stat-label">
                                        Classes attended
                                    </span>

                                </div>

                            </div>



                            <div className="attendance-stat">

                                <span className="dot dot-risk" />

                                <div>

                                    <span className="attendance-stat-value">
                                        {attendance.absent}
                                    </span>

                                    <span className="attendance-stat-label">
                                        Classes missed
                                    </span>

                                </div>

                            </div>



                            <div className="attendance-stat">

                                <span
                                    className={`dot dot-${attendanceTier.tone}`}
                                />

                                <div>

                                    <span className="attendance-stat-value">
                                        {attendanceTier.label}
                                    </span>

                                    <span className="attendance-stat-label">
                                        Current standing
                                    </span>

                                </div>

                            </div>

                        </div>

                    </div>

                </div>



                {/* Attendance bar chart */}

                <div className="dashboard-card">

                    <div className="section-header">

                        <h2>
                            Attendance Breakdown
                        </h2>

                        <p>
                            Present versus missed classes
                        </p>

                    </div>


                    <ResponsiveContainer
                        width="100%"
                        height={270}
                    >

                        <BarChart
                            data={attendanceBreakdownData}
                        >

                            <CartesianGrid
                                strokeDasharray="3 3"
                            />

                            <XAxis
                                dataKey="name"
                            />

                            <YAxis
                                allowDecimals={false}
                            />

                            <Tooltip />


                            <Bar
                                dataKey="Present"
                                name="Present"
                                fill="var(--accent)"
                                radius={[8, 8, 0, 0]}
                            />


                            <Bar
                                dataKey="Absent"
                                name="Absent"
                                fill="var(--text-primary)"
                                radius={[8, 8, 0, 0]}
                            />

                        </BarChart>

                    </ResponsiveContainer>

                </div>

            </section>



            {/* =====================================================
                ATTENDANCE QUICK CARDS
            ====================================================== */}

            <section className="mini-stat-grid">


                <div className="mini-stat-card">

                    <span className="mini-stat-icon">
                        ✓
                    </span>

                    <div>

                        <span>
                            Present
                        </span>

                        <strong>
                            {attendance.present}
                        </strong>

                    </div>

                </div>



                <div className="mini-stat-card">

                    <span className="mini-stat-icon">
                        ✕
                    </span>

                    <div>

                        <span>
                            Absent
                        </span>

                        <strong>
                            {attendance.absent}
                        </strong>

                    </div>

                </div>



                <div className="mini-stat-card">

                    <span className="mini-stat-icon">
                        %
                    </span>

                    <div>

                        <span>
                            Attendance
                        </span>

                        <strong>
                            {attendance.percentage}%
                        </strong>

                    </div>

                </div>



                <div className="mini-stat-card">

                    <span className="mini-stat-icon">
                        🎯
                    </span>

                    <div>

                        <span>
                            Standing
                        </span>

                        <strong>
                            {attendanceTier.label}
                        </strong>

                    </div>

                </div>

            </section>



            {/* =====================================================
                ASSESSMENT PERFORMANCE GRAPH
            ====================================================== */}

            



            {/* =====================================================
                SCORE DISTRIBUTION + SUMMARY
            ====================================================== */}

            <section className="dashboard-grid">
<section className="dashboard-card performance-chart-card">
                <div className="section-header">
                    <h2>
                        Assessment Performance
                    </h2>
                    <p>
                        Score achieved in each assessment
                    </p>
                </div>
                {assessmentChartData.length === 0 ? (

                    <EmptyState
                        icon="📄"
                        title="No assessments attempted yet"
                        body="Your assessment scores will appear here."
                    />

                ) : (

                    <ResponsiveContainer
                        width="100%"
                        height={350}
                    >

                        <BarChart
                            data={assessmentChartData}
                            margin={{
                                top: 10,
                                right: 20,
                                left: 0,
                                bottom: 10,
                            }}
                        >

                            <CartesianGrid
                                strokeDasharray="3 3"
                            />

                            <XAxis
                                dataKey="name"
                            />

                            <YAxis
                                domain={[0, 100]}
                                tickFormatter={(value) =>
                                    `${value}%`
                                }
                            />

                            <Tooltip
                                formatter={(value) => [
                                    `${value}%`,
                                    "Score",
                                ]}
                                labelFormatter={(label) => {

                                    const item =
                                        assessmentChartData.find(
                                            (x) =>
                                                x.name ===
                                                label
                                        );

                                    return (
                                        item?.title ||
                                        label
                                    );
                                }}
                            />


                            <Bar
                                dataKey="score"
                                name="Score"
                                fill="var(--accent)"
                                radius={[8, 8, 0, 0]}
                            />

                        </BarChart>

                    </ResponsiveContainer>

                )}

            </section>

                {/* Score distribution */}

                <div className="dashboard-card">

                    <div className="section-header">

                        <h2>
                            Score Distribution
                        </h2>

                        <p>
                            How your assessments are distributed
                        </p>

                    </div>


                    {submissions.length === 0 ? (

                        <EmptyState
                            icon="📊"
                            title="No score data"
                            body="Complete an assessment to see your score distribution."
                        />

                    ) : (

                        <ResponsiveContainer
                            width="100%"
                            height={300}
                        >

                            <BarChart
                                data={assessmentRangeData}
                            >

                                <CartesianGrid
                                    strokeDasharray="3 3"
                                />

                                <XAxis
                                    dataKey="range"
                                />

                                <YAxis
                                    allowDecimals={false}
                                />

                                <Tooltip />


                                <Bar
                                    dataKey="count"
                                    name="Assessments"
                                    fill="var(--accent)"
                                    radius={[8, 8, 0, 0]}
                                />

                            </BarChart>

                        </ResponsiveContainer>

                    )}

                </div>



                {/* Assessment summary */}

                <div className="dashboard-card">

                    <div className="section-header">

                        <h2>
                            Assessment Summary
                        </h2>

                        <p>
                            Quick view of your academic performance
                        </p>

                    </div>


                    <div className="summary-stat-grid">


                        <div className="summary-stat">

                            <span>
                                Average
                            </span>

                            <strong>
                                {assessments.averageScorePercent}%
                            </strong>

                        </div>


                        <div className="summary-stat">

                            <span>
                                Best
                            </span>

                            <strong>
                                {assessments.bestScorePercent}%
                            </strong>

                        </div>


                        <div className="summary-stat">

                            <span>
                                Lowest
                            </span>

                            <strong>
                                {submissions.length
                                    ? `${lowestScore}%`
                                    : "—"}
                            </strong>

                        </div>


                        <div className="summary-stat">

                            <span>
                                Grade
                            </span>

                            <strong>
                                {overallGrade}
                            </strong>

                        </div>


                        <div className="summary-stat">

                            <span>
                                Attempted
                            </span>

                            <strong>
                                {assessments.totalAttempted}
                            </strong>

                        </div>


                        <div className="summary-stat">

                            <span>
                                Highest
                            </span>

                            <strong>
                                {submissions.length
                                    ? `${highestScore}%`
                                    : "—"}
                            </strong>

                        </div>

                    </div>

                </div>

            </section>



            {/* =====================================================
                ASSESSMENT DETAILS
            ====================================================== */}

            <section className="performance-section">

                <div className="section-header">

                    <h2>
                        Assessment Details
                    </h2>

                    <p>
                        Detailed results for every assessment
                    </p>

                </div>


                {submissions.length === 0 ? (

                    <EmptyState
                        icon="📄"
                        title="No assessments attempted yet"
                        body="Assessments you complete will show up here with your score."
                    />

                ) : (

                    <div className="assessment-grid">

                        {submissions.map(
                            (submission) => {

                                const score =
                                    submission.percentageScore ??
                                    submission.percentage ??
                                    0;

                                const tier =
                                    getScoreTier(score);


                                return (

                                    <div
                                        className="assessment-card"
                                        key={submission._id}
                                    >


                                        <div className="assessment-card-top">

                                            <span className="assessment-title">
                                                {submission.title}
                                            </span>

                                            <span
                                                className={`score-pill tone-${tier}`}
                                            >
                                                {score}%
                                            </span>

                                        </div>


                                        <div className="assessment-bar-track">

                                            <div
                                                // className={`assessment-bar-fill tone-${tier}`}
                                                className={`assessment-bar-fill`}
                                                style={{
                                                    width: `${score}%`,
                                                }}
                                            />

                                        </div>


                                        <div className="assessment-card-bottom">

                                            <span className="score-badge">

                                                <strong>
                                                    Score:
                                                </strong>{" "}

                                                {submission.score}

                                            </span>

                                        </div>

                                    </div>

                                );
                            }
                        )}

                    </div>

                )}

            </section>



            {/* =====================================================
                TRAINER FEEDBACK ANALYTICS
            ====================================================== */}

            <section className="dashboard-grid">


                {/* Rating graph */}

                <div className="dashboard-card">

                    <div className="section-header">

                        <h2>
                            Trainer Ratings
                        </h2>

                        <p>
                            Distribution of trainer feedback ratings
                        </p>

                    </div>


                    {feedback.length === 0 ? (

                        <EmptyState
                            icon="💬"
                            title="No feedback yet"
                            body="Trainer ratings will appear here."
                        />

                    ) : (

                        <ResponsiveContainer
                            width="100%"
                            height={280}
                        >

                            <BarChart
                                data={feedbackRatingData}
                            >

                                <CartesianGrid
                                    strokeDasharray="3 3"
                                />

                                <XAxis
                                    dataKey="rating"
                                />

                                <YAxis
                                    allowDecimals={false}
                                />

                                <Tooltip />


                                <Bar
                                    dataKey="count"
                                    name="Ratings"
                                    fill="var(--accent)"
                                    radius={[8, 8, 0, 0]}
                                />

                            </BarChart>

                        </ResponsiveContainer>

                    )}

                </div>



                {/* Feedback summary */}

                <div className="dashboard-card">


                    <div className="section-header">

                        <h2>
                            Feedback Overview
                        </h2>

                        <p>
                            Your trainer feedback summary
                        </p>

                    </div>


                    <div className="feedback-big-rating">

                        <span className="feedback-rating-number">
                            {feedbackAverage}
                        </span>

                        <span className="feedback-stars">
                            ★★★★★
                        </span>

                        <span>
                            Average trainer rating
                        </span>

                    </div>


                    <div className="feedback-total">

                        <strong>
                            {feedback.length}
                        </strong>

                        <span>
                            Total feedback entries
                        </span>

                    </div>

                </div>

            </section>



            {/* =====================================================
                TRAINER FEEDBACK DETAILS
            ====================================================== */}

            <section className="performance-section">

                <div className="section-header">

                    <h2>
                        Trainer Feedback
                    </h2>

                    <p>
                        Notes from the trainers who've
                        worked with you
                    </p>

                </div>


                {feedback.length === 0 ? (

                    <EmptyState
                        icon="💬"
                        title="No trainer feedback yet"
                        body="Feedback from your trainers will appear here."
                    />

                ) : (

                    <div className="feedback-timeline">

                        {feedback.map((item) => (

                            <div
                                className="feedback-card"
                                key={item._id}
                            >


                                <div className="feedback-avatar">

                                    {initials(
                                        item.trainer?.name
                                    )}

                                </div>


                                <div className="feedback-body">


                                    <div className="feedback-header">

                                        <div>

                                            <strong>
                                                {item.trainer?.name ||
                                                    "Unknown Trainer"}
                                            </strong>


                                            {item.class && (

                                                <div className="feedback-class">

                                                    Class{" "}
                                                    {
                                                        item.class
                                                            .classNumber
                                                    }

                                                    :{" "}

                                                    {
                                                        item.class
                                                            .title
                                                    }

                                                </div>

                                            )}

                                        </div>


                                        <span className="feedback-date">

                                            {new Date(
                                                item.date
                                            ).toLocaleDateString(
                                                undefined,
                                                {
                                                    day: "2-digit",
                                                    month: "short",
                                                    year: "numeric",
                                                }
                                            )}

                                        </span>

                                    </div>


                                    <div className="feedback-rating">

                                        {"★".repeat(
                                            Number(
                                                item.rating || 0
                                            )
                                        )}

                                        {"☆".repeat(
                                            Math.max(
                                                0,
                                                5 -
                                                    Number(
                                                        item.rating ||
                                                            0
                                                    )
                                            )
                                        )}

                                        <span>
                                            {" "}
                                            (
                                            {item.rating}/5)
                                        </span>

                                    </div>


                                    <p>
                                        {item.comments}
                                    </p>

                                </div>

                            </div>

                        ))}

                    </div>

                )}

            </section>

        </div>
    );
}


/* =====================================================
   EMPTY STATE
===================================================== */

function EmptyState({
    icon,
    title,
    body,
}) {

    return (

        <div className="empty-state">

            <span className="empty-icon">
                {icon}
            </span>

            <strong>
                {title}
            </strong>

            <p>
                {body}
            </p>

        </div>

    );
}


















// import { useStu } from "../../../hooks/useStu";
// import "./PerformancePage.css";


// import {
//     RadialBarChart,
//     RadialBar,
//     ResponsiveContainer,
// } from "recharts";

// function getAttendanceTier(pct) {
//     if (pct >= 90) return { label: "Excellent", tone: "excellent" };
//     if (pct >= 75) return { label: "Good", tone: "good" };
//     if (pct >= 60) return { label: "Needs attention", tone: "fair" };
//     return { label: "At risk", tone: "risk" };
// }

// function getScoreTier(pct) {
//     if (pct >= 90) return "gold";
//     if (pct >= 75) return "good";
//     if (pct >= 60) return "fair";
//     return "risk";
// }

// function getOverallGrade(avgPct) {
//     if (avgPct >= 90) return "A+";
//     if (avgPct >= 80) return "A";
//     if (avgPct >= 70) return "B";
//     if (avgPct >= 60) return "C";
//     return "D";
// }

// function initials(name = "") {
//     return name
//         .split(" ")
//         .map((part) => part[0])
//         .filter(Boolean)
//         .slice(0, 2)
//         .join("")
//         .toUpperCase();
// }

// export default function PerformancePage({ token }) {
//     const { 
//         myPerformance 

//     } = useStu(token);

//     const performance = myPerformance?.data;



//     if (!performance) {
//         return (
//             <div className="performance-page">
//                 <div className="performance-loading">
//                     <div className="loading-ring" />
//                     <span>Loading your performance...</span>
//                 </div>
//             </div>
//         );
//     }

//     const { attendance, assessments, feedback } = performance;
//     const attendanceTier = getAttendanceTier(attendance.percentage);
//     const overallGrade = getOverallGrade(assessments.averageScorePercent);
//     const ringStyle = {
//         "--ring-value": attendance.percentage,
//     };
//     const attendanceChartData = [
//     {
//         name: "Attendance",
//         value: attendance.percentage,
//         fill: "var(--primary-color)",
//     },
// ];

//     return (
//         <div className="performance-page">
//             {/* Hero */}
//             <header className="performance-hero">
//                 <div className="hero-copy">
//                     <span className="hero-eyebrow">Performance overview</span>
//                     <h1>My Performance</h1>
//                     <p>Track your attendance, assessments and trainer feedback in one place.</p>
//                 </div>

//                 <div className="hero-grade" title="Overall grade, based on your average assessment score">
//                     <span className="hero-grade-label">Overall grade</span>
//                     <span className="hero-grade-value">{overallGrade}</span>
//                 </div>
//             </header>

//             {/* KPI cards */}
//             <section className="kpi-grid">
//                 <div className="kpi-card">
//                     <div className="kpi-icon accent-blue">🗓️</div>
//                     <span className="kpi-label">Total Classes</span>
//                     <span className="kpi-value">{attendance.totalClasses}</span>
//                 </div>

//                 <div className={`kpi-card accent-${attendanceTier.tone}`}>
//                     <div className={`kpi-icon accent-${attendanceTier.tone}`}>✓</div>
//                     <span className="kpi-label">Attendance</span>
//                     <span className="kpi-value">{attendance.percentage}%</span>
//                     <span className={`kpi-subtitle tone-${attendanceTier.tone}`}>
//                         {attendanceTier.label}
//                     </span>
//                 </div>

//                 <div className="kpi-card accent-gold">
//                     <div className="kpi-icon accent-gold">📝</div>
//                     <span className="kpi-label">Assessments</span>
//                     <span className="kpi-value">{assessments.totalAttempted}</span>
//                     <span className="kpi-subtitle">
//                         Average {assessments.averageScorePercent}%
//                     </span>
//                 </div>

//                 <div className="kpi-card accent-gold">
//                     <div className="kpi-icon accent-gold">🏆</div>
//                     <span className="kpi-label">Best Score</span>
//                     <span className="kpi-value">{assessments.bestScorePercent}%</span>
//                     <span className="kpi-subtitle">Personal best</span>
//                 </div>
//             </section>

//             {/* Attendance */}
//             <section className="performance-section">
//                 <div className="attendance-panel">

//     {/* Attendance Graph */}
//     <div className="attendance-chart-wrap">
//         <ResponsiveContainer width="100%" height={220}>
//             <RadialBarChart
//                 cx="50%"
//                 cy="50%"
//                 innerRadius="72%"
//                 outerRadius="92%"
//                 barSize={14}
//                 startAngle={90}
//                 endAngle={-270}
//                 data={attendanceChartData}
//             >
//                 <RadialBar
//                     background
//                     dataKey="value"
//                     cornerRadius={10}
//                 />

//                 {/* Center content */}
//                 <text
//                     x="50%"
//                     y="47%"
//                     textAnchor="middle"
//                     dominantBaseline="middle"
//                     className="attendance-chart-value"
//                 >
//                     {attendance.percentage}%
//                 </text>

//                 <text
//                     x="50%"
//                     y="61%"
//                     textAnchor="middle"
//                     dominantBaseline="middle"
//                     className="attendance-chart-label"
//                 >
//                     Attendance
//                 </text>
//             </RadialBarChart>
//         </ResponsiveContainer>
//     </div>

//     {/* Statistics */}
//     <div className="performance-attendance-stats">

//         <div className="attendance-stat">
//             <span className="dot dot-good" />

//             <div>
//                 <span className="attendance-stat-value">
//                     {attendance.present}
//                 </span>

//                 <span className="attendance-stat-label">
//                     Classes attended
//                 </span>
//             </div>
//         </div>

//         <div className="attendance-stat">
//             <span className="dot dot-risk" />

//             <div>
//                 <span className="attendance-stat-value">
//                     {attendance.absent}
//                 </span>

//                 <span className="attendance-stat-label">
//                     Classes missed
//                 </span>
//             </div>
//         </div>

//         <div className="attendance-stat">
//             <span className={`dot dot-${attendanceTier.tone}`} />

//             <div>
//                 <span className="attendance-stat-value">
//                     {attendanceTier.label}
//                 </span>

//                 <span className="attendance-stat-label">
//                     Current standing
//                 </span>
//             </div>
//         </div>

//     </div>
// </div>
//             </section>

//             {/* Assessments */}
//             <section className="performance-section">
//                 <div className="section-header">
//                     <h2>Assessments</h2>
//                     <p>Every assessment you've submitted, most recent first</p>
//                 </div>

//                 {assessments.submissions.length === 0 ? (
//                     <EmptyState
//                         icon="📄"
//                         title="No assessments attempted yet"
//                         body="Assessments you complete will show up here with your score."
//                     />
//                 ) : (
//                     <div className="assessment-grid">
//                         {/* <pre>{JSON.stringify(assessments, null, 2)}</pre> */}
//                         {assessments.submissions.map((submission) => {
//                             const tier = getScoreTier(submission.percentage);
//                             return (
//                                 <div className="assessment-card" key={submission._id}>
//                                     <div className="assessment-card-top">
//                                         <span className="assessment-title">{submission.title}</span>
//                                         <span className={`score-pill tone-${tier}`}>
//                                             {submission.percentageScore}%
//                                         </span>
//                                     </div>

//                                     <div className="assessment-bar-track">
//                                         <div
//                                             className={`assessment-bar-fill tone-${tier}`}
//                                             style={{ width: `${submission.percentageScore}%` }}
//                                         />
//                                     </div>

//                                     <div className="assessment-card-bottom">
//                                         <span className="score-badge">
//                                             <strong>Score:</strong> {submission.score}
//                                         </span>
//                                         {/* <span>
//                                             {new Date(submission.submittedAt).toLocaleDateString(
//                                                 undefined,
//                                                 { day: "2-digit", month: "short", year: "numeric" }
//                                             )}
//                                         </span> */}
//                                     </div>
//                                 </div>
//                             );
//                         })}
//                     </div>
//                 )}
//             </section>

//             {/* Feedback */}
//             {/* <section className="performance-section">
//                 <div className="section-header">
//                     <h2>Trainer Feedback</h2>
//                     <p>Notes from the trainers who've worked with you</p>
//                 </div>

//                 {feedback.length === 0 ? (
//                     <EmptyState
//                         icon="💬"
//                         title="No trainer feedback yet"
//                         body="Feedback from your trainers will appear here."
//                     />
//                 ) : (
//                     <div className="feedback-timeline">
//                         {feedback.map((item) => (
//                             <div className="feedback-card" key={item._id}>
//                                 <div className="feedback-avatar">{initials(item.trainerName)}</div>

//                                 <div className="feedback-body">
//                                     <div className="feedback-header">
//                                         <strong>{item.trainerName}</strong>
//                                         <span className="feedback-date">
//                                             {new Date(item.createdAt).toLocaleDateString(
//                                                 undefined,
//                                                 { day: "2-digit", month: "short", year: "numeric" }
//                                             )}
//                                         </span>
//                                     </div>
//                                     <p>{item.feedback}</p>
//                                 </div>
//                             </div>
//                         ))}
//                     </div>
//                 )}
//             </section> */}
//             {/* Feedback */}
//             <section className="performance-section">
//                 <div className="section-header">
//                     <h2>Trainer Feedback</h2>
//                     <p>Notes from the trainers who've worked with you</p>
//                 </div>

//                 {feedback.length === 0 ? (
//                     <EmptyState
//                         icon="💬"
//                         title="No trainer feedback yet"
//                         body="Feedback from your trainers will appear here."
//                     />
//                 ) : (
//                     <div className="feedback-timeline">
//                         {feedback.map((item) => (
//                             <div className="feedback-card" key={item._id}>
//                                 <div className="feedback-avatar">
//                                     {initials(item.trainer?.name)}
//                                 </div>

//                                 <div className="feedback-body">
//                                     <div className="feedback-header">
//                                         <div>
//                                             <strong>{item.trainer?.name || "Unknown Trainer"}</strong>

//                                             {item.class && (
//                                                 <div className="feedback-class">
//                                                     Class {item.class.classNumber}: {item.class.title}
//                                                 </div>
//                                             )}
//                                         </div>

//                                         <span className="feedback-date">
//                                             {new Date(item.date).toLocaleDateString(undefined, {
//                                                 day: "2-digit",
//                                                 month: "short",
//                                                 year: "numeric",
//                                             })}
//                                         </span>
//                                     </div>

//                                     <div className="feedback-rating">
//                                         {"★".repeat(item.rating)}
//                                         {"☆".repeat(5 - item.rating)}
//                                         <span> ({item.rating}/5)</span>
//                                     </div>

//                                     <p>{item.comments}</p>
//                                 </div>
//                             </div>
//                         ))}
//                     </div>
//                 )}
//             </section>
//         </div>
//     );
// }

// function EmptyState({ icon, title, body }) {
//     return (
//         <div className="empty-state">
//             <span className="empty-icon">{icon}</span>
//             <strong>{title}</strong>
//             <p>{body}</p>
//         </div>
//     );
// }













// // import { useStu } from "../../../hooks/useStu";

// // import "./PerformancePage.css";

// // export default function PerformancePage({ token }) {
// //     // const { myPerformance } = useStu();
// //       const {
// //         myPerformance
// //     } = useStu(token);


// //     const performance = myPerformance?.data;

// //     if (!performance) {
// //         return (
// //             <div className="performance-page">
// //                 <div className="performance-loading">
// //                     Loading performance...
// //                 </div>
// //             </div>
// //         );
// //     }

// //     const { attendance, assessments, feedback } = performance;

// //     return (
// //         <div className="performance-page">

// //             <div className="performance-header">
// //                 <h1>My Performance</h1>
// //                 <p>
// //                     Track your attendance, assessments and trainer feedback.
// //                 </p>
// //             </div>

// //             {/* Attendance */}

// //             <section className="performance-section">
// //                 <h2>Attendance</h2>

// //                 <div className="stats-grid">

// //                     <div className="stat-card">
// //                         <span>Total Classes</span>
// //                         <h2>{attendance.totalClasses}</h2>
// //                     </div>

// //                     <div className="stat-card green">
// //                         <span>Present</span>
// //                         <h2>{attendance.present}</h2>
// //                     </div>

// //                     <div className="stat-card red">
// //                         <span>Absent</span>
// //                         <h2>{attendance.absent}</h2>
// //                     </div>

// //                     <div className="stat-card blue">
// //                         <span>Attendance %</span>
// //                         <h2>{attendance.percentage}%</h2>
// //                     </div>

// //                 </div>

// //                 <div className="attendance-progress">

// //                     <div
// //                         className="attendance-progress-fill"
// //                         style={{
// //                             width: `${attendance.percentage}%`,
// //                         }}
// //                     />

// //                 </div>

// //             </section>

// //             {/* Assessments */}

// //             <section className="performance-section">
// //                 <h2>Assessments</h2>

// //                 <div className="stats-grid">

// //                     <div className="stat-card">
// //                         <span>Attempted</span>
// //                         <h2>{assessments.totalAttempted}</h2>
// //                     </div>

// //                     <div className="stat-card purple">
// //                         <span>Average Score</span>
// //                         <h2>{assessments.averageScorePercent}%</h2>
// //                     </div>

// //                     <div className="stat-card orange">
// //                         <span>Best Score</span>
// //                         <h2>{assessments.bestScorePercent}%</h2>
// //                     </div>

// //                 </div>

// //                 <div className="table-wrapper">

// //                     <table className="performance-table">

// //                         <thead>
// //                             <tr>
// //                                 <th>Assessment</th>
// //                                 <th>Score</th>
// //                                 <th>Percentage</th>
// //                                 <th>Date</th>
// //                             </tr>
// //                         </thead>

// //                         <tbody>

// //                             {assessments.submissions.length === 0 ? (
// //                                 <tr>
// //                                     <td colSpan="4" className="empty-row">
// //                                         No assessments attempted yet.
// //                                     </td>
// //                                 </tr>
// //                             ) : (
// //                                 assessments.submissions.map((submission) => (
// //                                     <tr key={submission._id}>
// //                                         <td>{submission.title}</td>
// //                                         <td>
// //                                             {submission.score} /{" "}
// //                                             {submission.maxScore}
// //                                         </td>
// //                                         <td>{submission.percentage}%</td>
// //                                         <td>
// //                                             {new Date(
// //                                                 submission.submittedAt
// //                                             ).toLocaleDateString()}
// //                                         </td>
// //                                     </tr>
// //                                 ))
// //                             )}

// //                         </tbody>

// //                     </table>

// //                 </div>

// //             </section>

// //             {/* Feedback */}

// //             <section className="performance-section">

// //                 <h2>Trainer Feedback</h2>

// //                 {feedback.length === 0 ? (
// //                     <div className="empty-feedback">
// //                         No feedback available yet.
// //                     </div>
// //                 ) : (
// //                     <div className="feedback-list">

// //                         {feedback.map((item) => (
// //                             <div
// //                                 className="feedback-card"
// //                                 key={item._id}
// //                             >
// //                                 <div className="feedback-header">
// //                                     <strong>{item.trainerName}</strong>

// //                                     <span>
// //                                         {new Date(
// //                                             item.createdAt
// //                                         ).toLocaleDateString()}
// //                                     </span>
// //                                 </div>

// //                                 <p>{item.feedback}</p>

// //                             </div>
// //                         ))}

// //                     </div>
// //                 )}

// //             </section>

// //         </div>
// //     );
// // }